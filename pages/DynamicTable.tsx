...
export default function DynamicTable() {
  ...
  const calculateFinalData = () => {
    const totalSum = newRow.reduce((acc, current) => acc + current.totalCost, 0);
    const totalQuantity = newRow.reduce((acc, current) => acc + current.quantity, 0);
    const toalToken = newRow.reduce((acc, current) => acc + current.pricePerToken, 0);
      ...
      <div className='p-3 pb-8' style={{ background: '#060508' }}>
        ...
        <div className='sm:px-3 px-2 py-2 flex items-start mr-0 mb-8 bg-[#363738]'>
          ...
          <div className='leading-5 font-[Oxanium-Regular] sm:text-sm text-xs'>
            <p>Cost average, DCA, or stock average calculator is a tool that helps you calculate the average purchase price of a crypto or any other asset bought at different times and at different prices using dollar-cost averaging strategy.</p>
          </div>
        </div>
        ...
        <div className='flex items-center justify-start mb-6'>
          <button onClick={e => calculateFinalData().avarage > 0 && setShowCalculation(true)} className='rounded-md font-[Oxanium-SemiBold] px-3 py-2 flex items-center bg-[#27AE60] hover:text-[#27AE60] hover:bg-[#DFE6E9] duration-150'
        ...
        {showCalculation && <div className='flex items-center justify-center'>
          <div className='rounded-xl xl:w-1/2 md:w-2/3 w-full sm:px-8 px-2 sm:py-5 py-3' style={{ backgroundImage: "linear-gradient(to right, #2C2C54, #706FD3)" }}>
            <div className='flex justify-between'><p className='font-[Oxanium-SemiBold] lg:text-lg sm:text-base text-xs md:mb-4 mb-2'>THE AVERAGE PURCHASE PRICE PER ASSET IS</p>
              <div onMouseOver={toggleResetButtonHover} onMouseOut={toggleResetButtonHover}>
                <img src={isResetButtonHover ? IMAGES.resetButtonHoverIcon : IMAGES.resetButtonIcon} alt="" className='sm:w-5 w-4' />
              </div>
            </div>
            <h1 className='font-[Oxanium-SemiBold] mb-2 lg:text-4xl md:text-3xl sm:text-2xl text-xl'>${calculateFinalData().avarage}</h1>
            <p className='font-[Oxanium-Regular] mb-2 md:text-base sm:text-sm text-xs'>With the total purchase quantity of <span className='font-[Oxanium-Bold] underline'>{calculateFinalData().totalQuantity} tokens</span></p>
            <p className='font-[Oxanium-Regular] mb-2 md:text-base sm:text-sm text-xs'>And the total purchase cost amounting to <span className='font-[Oxanium-Bold] underline'>${calculateFinalData().totalSum}</span></p>
          </div>
        </div>
        }
